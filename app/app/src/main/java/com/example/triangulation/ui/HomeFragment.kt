...
class HomeFragment : androidx.fragment.app.Fragment(), android.hardware.SensorEventListener, OsmAndAidlHelper.OsmAndAidlListener {
            ...
            btnIntersection.setOnClickListener {
                ...
                if (targetLat != null && targetLon != null) {
                    ...
                    try {
                        ...
                        showVerboseToast("OsmAnd+ hot/cold intent fired")
                    } catch (e: Exception) {
    ...
    private fun saveCurrentReading() {
        if (currentLat != null && currentLon != null) {
            ...
            saveState()
            updatePointsList()
            showVerboseToast("Reading saved. Drawing silently on Map...")
    ...
    private fun checkHorizonWarning(targetLat: Double, targetLon: Double) {
        if (selectedLocations.isNotEmpty()) {
            ...
            val sharedPrefs = requireActivity().getSharedPreferences("triangulation_prefs", Context.MODE_PRIVATE)
            val threshold = sharedPrefs.getInt("horizon_threshold", 2000)

            if (distKm > threshold) {
                activity?.runOnUiThread {
                    Toast.makeText(requireContext(), "Warning: Intersection is very far (> $threshold km), possibly over the horizon", Toast.LENGTH_LONG).show()
                }
            }
        }
    }

    private fun showVerboseToast(message: String) {
        val verbose = requireActivity().getSharedPreferences("triangulation_prefs", Context.MODE_PRIVATE).getBoolean("verbose_mode", false)
        if (verbose) Toast.makeText(requireContext(), message, Toast.LENGTH_SHORT).show()
    }
    ...
    private fun showOsmAndPluginAlert() {
        requireActivity().runOnUiThread {
            ...
            android.app.AlertDialog.Builder(requireContext())
                ...
                .setCancelable(false)
                .setMessage("Failed to interact with OsmAnd.\n\nPlease ensure that requireContext() Triangulation app is enabled in OsmAnd's plugin settings.")
    ...
    override fun onOsmAndServiceConnected() {
        ...
        requireActivity().runOnUiThread {
            showVerboseToast("Connected to OsmAnd API")
        }
        if (!osmandHelper.addContextMenuButton(1001, "Take Back-Azimuth", "")) showVerboseToast("Failed to add context menu to OsmAnd")
    }

    override fun onOsmAndServiceDisconnected() {
        Log.d("Triangulation", "OsmAnd Service Disconnected.")
        requireActivity().runOnUiThread {
            showVerboseToast("Disconnected from OsmAnd API")
        }
    }
    ...
    override fun onContextMenuButtonClicked(buttonId: Int, pointId: String?, layerId: String?) {
        ...
        Handler(Looper.getMainLooper()).post {
            val launchIntent = Intent(requireContext(), com.example.triangulation.MainActivity::class.java)
            ...
            try {
                if (pointId != null && pointId.contains(",")) {
                    ...
                    launchIntent.putExtra("lat", lat)
                    launchIntent.putExtra("lon", lon)
                } else {
                    launchIntent.putExtra("pointId", pointId)
                }
            } catch (e: Exception) {
                e.printStackTrace()
            }
                ...
                if (feature.has("geometry")) {
                    ...
                    if (geom.optString("type") == "Point") {
                        ...
                        if (feature.has("properties")) {
                            ...
                            desc = props.optString("name", props.optString("desc", ""))
                            img = props.optString("image", props.optString("img", ""))
                        }
            ...
            try {
                ...
                if (jsonStr != null) {
                    ...
                    if (pointsAdded > 0) {
                        saveState()
                        updatePointsList()
                        showVerboseToast("Imported $pointsAdded observations")
    ...
    private fun showSaveDialog(lat: Double, lon: Double, initialDesc: String?) {
        ...
        builder.setPositiveButton("Save") { dialog, _ ->
            ...
            showVerboseToast("Location saved to library")
        }
        builder.setNegativeButton("Cancel", null)
        builder.show()
    }
    ...
    private fun drawTriangulationPointsOnMap() {
        ...
        if (!aidlSuccess) {
            requireActivity().runOnUiThread { Toast.makeText(requireContext(), "Failed to send GPX to OsmAnd via AIDL", Toast.LENGTH_SHORT).show() }
        }
    }
    ...
    private fun refreshUIForCurrentLocation() {
        ...
        if (hasLocation && rawReceivedParameter != null) {
            ...
            requireActivity().title = "Loc: ${String.format("%.5f", currentLat)}, ${String.format("%.5f", currentLon)}"
        } else {
            requireActivity().title = "Triangulation - No Location"
        }
    }
